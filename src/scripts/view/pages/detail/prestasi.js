const DetailPrestasi = {
    async render() {
      ...
      return `
        <main id="detail-container" class="container mx-auto max-w-2xl px-4 py-6 transition-all duration-300 ease-in-out opacity-0 scale-95">
          ...
          <img src="${data.gambar}" alt="${data.judul}" class="w-full h-52 object-cover rounded-md shadow mb-4"/>
          <p class="text-sm text-gray-800 leading-relaxed whitespace-pre-line mb-6">${data.deskripsi}</p>
          ...
          <div class="mt-6">
            <button id="close-detail" class="inline-flex items-center px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg text-sm font-semibold transition-all duration-300 ease-in-out">
    ...
    async afterRender() {
      ...
      closeBtn?.addEventListener('click', () => {
        ...
        setTimeout(() => {
          window.location.hash = '#/ekstrakurikuler';
          window.dispatchEvent(new HashChangeEvent("hashchange")); // penting untuk SPA
        }, 300);
      });
    }
  };
