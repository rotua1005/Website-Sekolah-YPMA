        ...
        return kelasData.map((kelas, index) => {
            ...
            return `
                <tr class="border-t">
                    ...
                    <td class="py-4 px-6">${tahunAkademik ? tahunAkademik.tahun : 'N/A'}</td>
                    <td class="py-4 px-6">${tahunAkademik ? tahunAkademik.semester : 'N/A'}</td>
    ...
    attachRowEventListeners() {
        ...
        document.querySelectorAll('.kelola-btn').forEach((btn) => {
            btn.addEventListener('click', function () {
                ...
                // Simpan data kelas yang akan dikelola ke localStorage
                localStorage.setItem('kelasUntukAbsensi', JSON.stringify(selectedKelas));

                // Navigasi ke halaman Absensi 2
