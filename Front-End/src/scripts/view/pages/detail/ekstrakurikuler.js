const DetailEkstrakurikuler = {
    async render() {
        ...
        const id = hashSegments[hashSegments.length - 1]; // Use string id for API

        let data = null;
        try {
            // Fetch detail ekstrakurikuler by ID from API
            const response = await fetch(`http://localhost:5000/api/dashboardEkstrakurikuler/${id}`);
            if (!response.ok) throw new Error("Gagal fetch detail ekstrakurikuler");
            data = await response.json();
        } catch (e) {
            console.error("Error fetching detail ekstrakurikuler:", e);
            return `<div class="text-center mt-20 text-red-600 text-xl font-semibold">Gagal memuat data ekstrakurikuler. Silakan coba lagi nanti.</div>`;
        }
        ...
        // Fetch all ekstrakurikuler for related list
        let ekstrakurikulerList = [];
        try {
            const allResponse = await fetch('http://localhost:5000/api/dashboardEkstrakurikuler');
            if (allResponse.ok) {
                ekstrakurikulerList = await allResponse.json();
            }
        } catch (e) {
            ekstrakurikulerList = [];
        }
        // Filter out current and take 3 related
        const relatedEkstra = ekstrakurikulerList.filter(item => (item._id || item.id) != id).slice(0, 3);
            ...
            <main class="w-full mt-0 px-4 md:px-20">
                <section class="relative w-full mb-8 flex flex-col md:flex-row gap-8">
                    ...
                    <aside class="md:w-1/3">
                        ...
                        <div class="space-y-4">
                            ${relatedEkstra.map(ekstra => `
                                <div class="flex gap-4">
                                    <img src="${ekstra.gambar ? `${backendBaseUrl}${ekstra.gambar}` : '/placeholder-image.jpg'}" alt="${ekstra.nama}" class="w-20 h-16 object-cover rounded-md" />
                                    <div>
