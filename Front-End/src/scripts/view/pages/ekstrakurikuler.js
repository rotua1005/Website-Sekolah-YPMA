const Ekstrakurikuler = {
            ...
            if (list.length > 0) {
                return list.map((item, index) => `
                    <div class="berita-item bg-white p-4 rounded-xl shadow-md hover:shadow-xl transition-all duration-500 group" data-aos="fade-up-right">
                        ...
                        <a href="#/detail_ekstrakurikuler/${index}" class="inline-block bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded mt-4 transition read-more-btn">
                            Read More
                        </a>
                    </div>
                `).join('');
    ...
    async afterRender() {
        ...
        attachReadMoreListeners('.read-more-btn', ekstrakurikulerList, 'detail-ekstrakurikuler');
            ...
            if (searchInput && container) {
                searchInput.addEventListener('input', function () {
                    ...
                    const generateFilteredCards = (filtered) => {
                        ...
                        attachReadMoreListeners('.read-more-btn', filtered, 'detail-ekstrakurikuler');
                    };
