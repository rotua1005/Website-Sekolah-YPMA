...
const Kehadiran = {
    async fetchMonthlyAbsensiAdmin(kelas, waliKelas, year, monthIndex) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.error('Error fetching monthly attendance (admin):', errorData.message);
                alert(`Gagal mengambil data absensi bulanan: ${errorData.message}`);
                return [];
            }
        } catch (error) {
            console.error('Network error fetching monthly attendance (admin):', error);
    ...
    async render() {
        ...
        // Fetch absensi data from API (like wali kelas)
        const monthlyAbsensiData = await this.fetchMonthlyAbsensiAdmin(namaKelas, waliKelas, selectedYear, selectedMonthIndex);
