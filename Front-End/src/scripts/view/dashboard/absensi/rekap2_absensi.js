...
const Rekap2Absensi = {
    assignedClass: '',
    monthlyAbsensiData: [],
    allDataSiswa: [], // To store all student data fetched from the API
    ...
    async render() {
        ...
        await this.fetchAllSiswaData(); // NEW: Fetch student data from API

        // Fetch monthly attendance data based on URL hash parameters or defaults
        // Now calling with all necessary parameters for API fetch
    ...
    async afterRender() {
        ...
        const printButton = document.getElementById('printRekapBtn'); // Get the print button
        ...
        if (filterButton) {
            filterButton.addEventListener('click', async () => {
                ...
                // Get active academic year (needed for fetchMonthlyAbsensi)
                const tahunAkademikData = JSON.parse(localStorage.getItem('dataTahun')) || [];
                const tahunAkademikAktif = tahunAkademikData[tahunAkademikData.length - 1] || { tahun: '-', semester: '-' };

                // Fetch new attendance data based on the selected month and year from API
                ...
                // Re-filter student data (it's already fetched once, just re-use)
                const siswaSesuaiKelas = this.allDataSiswa.filter(siswa => String(siswa.kelas) === String(this.assignedClass));
    ...
    // NEW: Function to fetch all student data from the API
    async fetchAllSiswaData() {
        try {
            ...
            this.allDataSiswa = data; // Store the fetched data
        } catch (error) {
            console.error('Error fetching all student data:', error);
            this.allDataSiswa = []; // Ensure it's an empty array on error
        }
    },

    // MODIFIED: This function now fetches attendance data from the API
    ...
    updateRekapContent(siswaSesuaiKelas, days, selectedYear, selectedMonthIndex) {
        ...
        const h1Title = document.getElementById('rekapTitle'); // Use the ID for H1
        if (tbody && h1Title) {
            const selectedMonthName = this.getMonthName(selectedMonthIndex);
    ...
    // NEW: Function to handle printing the attendance recap
    printRekapAbsensi() {
        ...
        if (rekapTable && rekapTitle) {
            ...
            printWindow.document.write(`
                <style>
                    ...
                    .bg-green-200, .bg-red-200, .bg-yellow-200, .bg-gray-400 { background-color: #e2e8f0; } /* Adjust lighter colors for print */
                    .text-center { text-align: center; }
                    .font-bold { font-weight: bold; }
                </style>
            `);
            printWindow.document.write('</head><body>');
            printWindow.document.write('<h1>' + rekapTitle.innerHTML + '</h1>'); // Include the title
            printWindow.document.write(rekapTable.outerHTML); // Include the entire table
            printWindow.document.write('</body></html>');
            printWindow.document.close();
            printWindow.print();
        } else {
            console.error('Error: Rekap table or title not found for printing.');
            // You could implement a custom modal/message box here instead of an alert
            // For example: showInfoMessage('Rekap absensi tidak dapat dicetak. Tabel atau judul tidak ditemukan.');
        }
    },
};
