...
const DataKelasKepsek = {
                    ...
                    <div class="bg-white shadow-2xl rounded-lg p-8 mt-5">
                        ...
                        <div class="flex justify-end items-center mb-4">
                            <div class="flex space-x-4">
                                <select id="filterKelasDataKelasKepsek" class="border p-3 rounded-lg text-lg">
                                ...
                                <select id="filterTahunAkademikDataKelasKepsek" class="border p-3 rounded-lg text-lg">
                                    <option value="">Semua Tahun Akademik</option>
                                    </select>
                                <input type="text" id="searchWaliKelasKepsek" class="border p-3 rounded-lg text-lg" placeholder="Cari Nama Wali Kelas">
                            </div>
                        </div>
                        ...
                        <div class="shadow-xl rounded-lg p-6 overflow-x-auto">
                            <table class="w-full border shadow-lg rounded-lg text-lg">
                                ...
                                <tbody id="dataKelasTableKepsek" class="text-gray-700">
    ...
    async afterRender() {
        ...
        const dataTableBody = document.getElementById('dataKelasTableKepsek');
        const filterKelasSelect = document.getElementById('filterKelasDataKelasKepsek');
        const filterTahunAkademikSelect = document.getElementById('filterTahunAkademikDataKelasKepsek');
        const searchWaliKelasInput = document.getElementById('searchWaliKelasKepsek');
        ...
        async function populateTahunAkademikFilter() {
            try {
                ...
                console.error("Error populating Tahun Akademik filter for Data Kelas:", error);
            }
        }
        ...
        async function renderTable() {
            ...
            const search = searchWaliKelasInput.value;
            ...
            const url = `http://localhost:5000/api/datakelas?${queryParams.toString()}`;

            try {
                dataTableBody.innerHTML = `<tr><td colspan="5" class="text-center py-4">Loading data...</td></tr>`; // Show loading state
                const response = await fetch(url);
                if (!response.ok) {
                    throw new Error('Failed to fetch Data Kelas from the server');
                }
                const dataKelas = await response.json();

                if (dataKelas.length === 0) {
                    dataTableBody.innerHTML = `<tr><td colspan="5" class="text-center py-4">Tidak ada data Kelas yang ditemukan.</td></tr>`;
                    return;
                }

                dataTableBody.innerHTML = dataKelas.map((kelasEntry, index) => `
                    <tr class="border-t">
                        <td class="py-4 px-6">${index + 1}</td>
                        <td class="py-4 px-6">${kelasEntry.waliKelas.nama}</td>
                        <td class="py-4 px-6">${kelasEntry.kelas}</td>
                        <td class="py-4 px-6">${kelasEntry.jumlahSiswa}</td>
                        <td class="py-4 px-6">${kelasEntry.tahunAkademik || '-'}</td>
                    </tr>
                `).join('');

            } catch (error) {
                console.error('Error rendering Data Kelas table for Kepsek:', error);
        ...
        searchWaliKelasInput.addEventListener('input', renderTable);
